...
window.onload = function()
{
	...
	// The Pointer is a special component that is used to select other components on a form. When this function is
	// called, it means that a group of elements has been selected by the user on the current form.
	tools_fx[0] = {};
	...
	tools_fx[0].create = function(outline_div)
	{
		...
		if (width > 4 && height > 4)
		{
			...
			// move the outline div out of visible range - it will be put back once it is determined what elements have been selected
			...
			{
				// retrieve the x/y position of this object
				var this_x = objects[i].left;
				var this_y = objects[i].top;

				// retrieve the width and the height of this object
				var this_width = objects[i].width;
				var this_height = objects[i].height;

				// if the element fits entirely within the space defined by the outline div, it is added to the select group
				if (this_x >= x && (this_x + this_width) <= (x + width) && this_y >= y && (this_y + this_height) <= (y + height))
				{
					var index = select_group.length;

					// the select_group array stores the object, with the select_index array stores the object index
					select_group[index] = objects[i];
					select_index[index] = i;
				}
			}

			// get the name of the form that is being worked with
			var this_form = get_object(parent_name).form;

			// call the formspace set_outline_focus function, indicating that a group of elements on the current form have been selected
