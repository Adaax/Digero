...
function functions_dialog(data)
{
	...
	this.functions_div_search.addEventListener("keypress", function (this_functions, funcs)
	{
		return function(evt)
		{
			var i = 0;

			while (funcs[i].search(this.value) == -1 && i < funcs.length)
				i++;

			if (i < funcs.length)
				this_functions.highlight(i);
		}
		...
		for (var i = 0; i < this.funcs.length; i++)
		{
			...
			entry_div.addEventListener("dblclick", function (this_functions, func)
			{
				return function(evt)
				{
					...
					for (i = 0; i < main_formspace.tab_names.length; i++)
					{
						if (main_formspace.tab_names[i].innerHTML == func.file)
						{
							main_formspace.set_tab_focus(i);
							...
							var range = editor.find(func.code, {
							    wrap: true,
							    caseSensitive: true, 
							    wholeWord: true,
							    regExp: false,
